...
def test_simple_interaction_kernel(fieldset, mode):
    lons = [0.0, 0.1, 0.25, 0.45]
    lats = [0.0, 0.0, 0.0, 0.0]
    pset = ParticleSet(fieldset, pclass=ptype[mode], lon=lons, lat=lats)  # TODO: Add interactiondistance (0.2)
    pset.execute(DoNothing,  # TODO: add interactionkernel DummyMoveNeighbour
                 endtime=1., dt=1.)
