from parcels import Grid, ParticleSet, Field, ScipyParticle, JITParticle
import numpy as np
import pytest
...
def test_pset_create_lon_lat(grid, mode, npart=100):
    ...
    pset = ParticleSet(grid, lon=lon, lat=lat, pclass=ptype[mode])
...
def test_pset_create_line(grid, mode, npart=100):
    lon = np.linspace(0, 1, npart, dtype=np.float32)
    lat = np.linspace(1, 0, npart, dtype=np.float32)
    pset = ParticleSet.from_line(grid, size=npart, start=(0, 1), finish=(1, 0), pclass=ptype[mode])
    assert np.allclose([p.lon for p in pset], lon, rtol=1e-12)
    assert np.allclose([p.lat for p in pset], lat, rtol=1e-12)
...
def test_pset_create_field(grid, mode, npart=100):
    ...
    pset = ParticleSet.from_field(grid, size=npart, pclass=ptype[mode], start_field=K)
...
def test_pset_merge_inplace(grid, mode, npart=100):
    pset1 = ParticleSet(grid, pclass=ptype[mode],
                        lon=np.linspace(0, 1, npart, dtype=np.float32),
                        lat=np.linspace(1, 0, npart, dtype=np.float32))
    pset2 = ParticleSet(grid, pclass=ptype[mode],
                        lon=np.linspace(0, 1, npart, dtype=np.float32),
