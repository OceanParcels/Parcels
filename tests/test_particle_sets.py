...
def test_pset_create_fromparticlefile(fieldset, pset_mode, mode, restart, tmpdir):
    ...
    if pset_mode != 'nodes':
        pset = pset_type[pset_mode]['pset'](fieldset, lon=lon, lat=lat, depth=[4]*len(lon),
                                            pclass=TestParticle, p3=np.arange(len(lon)))
    else:
        idgen = GenerateID_Service(SequentialIdGenerator)
...
def test_pset_remove_index(fieldset, pset_mode, mode, npart=100):
    ...
    for ilon, ilat in zip(lon[::-1], lat[::-1]):
        ...
        plon = pset.lon[-1] if pset_mode != 'nodes' else p.data.lon
        plat = pset.lat[-1] if pset_mode != 'nodes' else p.data.lat
        assert(plon == ilon)
        assert(plat == ilat)
        p = p.data if pset_mode == 'nodes' else p
        pset.remove(p)
    assert(pset.size == 0)
...
def test_pset_multi_execute_delete(fieldset, pset_mode, mode, npart=10, n=5):
    ...
    for _ in range(n):
        pset.execute(k_add, runtime=1., dt=1.0)
        p = -1 if pset_mode != 'nodes' else pset.end().data
        pset.remove(p)
...
def test_from_field_exact_val(pset_mode, staggered_grid):
    ...
    pset = None
    if pset_mode != 'nodes':
        pset = pset_type[pset_mode]['pset'].from_field(fieldset, size=400, pclass=SampleParticle,
                                                       start_field=FMask, time=0)
    else:
        idgen = GenerateID_Service(SequentialIdGenerator)
        idgen.setDepthLimits(0., 1.0)
        idgen.setTimeLine(0.0, 1.0)
        c_lib_register = LibraryRegisterC()
        pset = pset_type[pset_mode]['pset'].from_field(fieldset, size=400, pclass=SampleParticle,
                                                       start_field=FMask, time=0, idgen=idgen, c_lib_register=c_lib_register)
