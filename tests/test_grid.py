...
def test_fieldset_from_data(xdim, ydim):
    ...
    u, v, lon, lat, depth, time = generate_fieldset(xdim, ydim)
...
def test_fieldset_from_nemo(xdim, ydim, tmpdir, filename='test_nemo'):
    ...
    u, v, lon, lat, depth, time = generate_fieldset(xdim, ydim)
    fieldset_out = FieldSet.from_data(u, lon, lat, v, lon, lat, depth, time)
...
def test_fieldset_from_file_subsets(indslon, indslat, tmpdir, filename='test_subsets'):
    ...
    filepath = tmpdir.join(filename)
    fieldsetfull = FieldSet.from_data(u, lon, lat, v, lon, lat, depth, time)
...
def test_moving_eddies_file_subsettime(indstime):
    ...
    fieldsetfull = FieldSet.from_nemo(fieldsetfile, extra_vars={'P': 'P'})
    fieldsetsub = FieldSet.from_nemo(fieldsetfile, extra_vars={'P': 'P'}, indices={'time': indstime})
