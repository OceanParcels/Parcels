...
def test_invalid_positions():
    ...
    j, i, coords = grid.get_spatial_hash().query([np.nan, np.inf], [np.nan, np.inf], curvilinear_point_in_cell)
    assert np.all(j == -3)
    assert np.all(i == -3)
...
def test_mixed_positions():
    ...
    y = [lat, np.nan]
    x = [lon, np.nan]
    j, i, coords = grid.get_spatial_hash().query(y, x, curvilinear_point_in_cell)
