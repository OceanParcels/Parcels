...
class KernelGenerator(ABC, ast.NodeVisitor):
    ...
    def visit_VectorFieldEvalNode(self, node):
        ...
        if '3DSigma' in node.field.obj.vector_type:
            statements_croco.append(c.Assign("particles->state[pnum]", f"temporal_interpolation({args[3]}, {args[2]}, 0, 0, H, &particles->xi[pnum*ngrid], &particles->yi[pnum*ngrid], &particles->zi[pnum*ngrid], &particles->ti[pnum*ngrid], &{node.var}, LINEAR, {node.field.obj.U.gridindexingtype.upper()})"))
