                              ...
                              double *particle_dlon, double *particle_dlat)
{
  ...
  double s_min = min(min(fabs(ds_x), fabs(ds_y)), fabs(ds_t / (dxdy)));
  ...
  *particle_dlat = (1.-rs_x)*(1.-rs_y) * py[0] + rs_x * (1.-rs_y) * py[1] + rs_x * rs_y * py[2] + (1.-rs_x)*rs_y * py[3] - *lat;
  ...
  if (*dt > 0){
    *dt = max(direction * s_min * dxdy, 1e-7);
  } else {
    *dt = min(direction * s_min * dxdy, 1e-7);
  }
...
static inline StatusCode calcAdvectionAnalytical_3D_JIT(CField *fu, CField *fv, CField *fw,
                              int *xi, int *yi, int *zi,
                              double *lon, double *lat, double *depth,
                              double *time, double *dt,
