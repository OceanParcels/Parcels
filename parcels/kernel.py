...
class Kernel(object):
    ...
    def __del__(self):
        ...
        if not self._lib == None:
            _ctypes.FreeLibrary(self._lib._handle) if platform == 'win32' else _ctypes.dlclose(self._lib._handle)
            del self._lib
            self._lib = None
            map(remove, [self.src_file, self.lib_file, self.log_file]) if path.isfile(self.lib_file) else None #We'll keep the logs.
    ...
    def remove_lib(self):
        ...
        if not self._lib == None:
            _ctypes.FreeLibrary(self._lib._handle) if platform == 'win32' else _ctypes.dlclose(elf._lib._handle)
            del self._lib
            self._lib = None
