...
class Kernel(object):
    ...
    def __del__(self):
        ...
        if self._lib is not None:
            ...
            del self._lib
            self._lib = None
            list(map(remove, [self.src_file, self.lib_file, self.log_file])) if path.isfile(self.lib_file) else None
    ...
    def remove_lib(self):
        ...
        if path.isfile(self.lib_file):
            list(map(remove, [self.src_file, self.lib_file, self.log_file]))
            basename = path.join(get_cache_dir(), self._cache_key)
            self.src_file = "%s.c" % basename
            self.lib_file = "%s.%s" % (basename, 'dll' if platform == 'win32' else 'so')
            self.log_file = "%s.log" % basename
