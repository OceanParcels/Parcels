...
def test_stommel_fieldset(mode):
    ...
    assert np.allclose([p.lon for p in psetRK4], [p.lon for p in psetRK45], atol=1e-3)
    assert np.allclose([p.lat for p in psetRK4], [p.lat for p in psetRK45], atol=1e-3)
