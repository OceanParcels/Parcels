...
try:
    ...
    raise EnvironmentError(
        "Parcels version can not be retrieved. Have you run "
...
class BaseParticleFile(ABC):
    ...
    def __init__(
        ...
        self.chunks = chunks
        self.lasttime_written = (
            None  # variable to check if time has been written already
        )
    ...
    def _create_variables_attribute_dict(self):
        ...
        if self.time_origin.calendar is not None:
            ...
            attrs["time"]["calendar"] = (
                "standard"
                if self.time_origin.calendar == "np_datetime64"
                else self.time_origin.calendar
            )
    ...
    def write(self, pset, time, deleted_only=False):
        ...
        if self.lasttime_written != time and (
            ...
            if deleted_only is not False:
                if type(deleted_only) not in [list, np.ndarray] and deleted_only in [
                    True,
                    1,
                ]:
                    indices_to_write = np.where(
                        np.isin(
                            pset.collection.getvardata("state"), [OperationCode.Delete]
                        )
                    )[0]
                ...
                indices_to_write = pset.collection._to_write_particles(
                    pset.collection._data, time
                )
            ...
            if len(indices_to_write) > 0:
                ...
                once_ids = np.where(
                    pset.collection.getvardata("once_written", indices_to_write) == 0
                )[0]
                ids_once = ids[once_ids]
                indices_to_write_once = indices_to_write[once_ids]
                pset.collection.setvardata(
                    "once_written", indices_to_write_once, np.ones(len(ids_once))
                )

                if self.maxids > len(self.obs_written):
                    self.obs_written = np.append(
                        self.obs_written,
                        np.zeros((self.maxids - len(self.obs_written)), dtype=int),
                    )
                ...
                if self.create_new_zarrfile:
                    if self.chunks is None:
                        ...
                        logger.warning(
                            f"Chunk size for trajectory ({self.chunks[0]}) is larger than length of initial set to write. "
                    ...
                    for var in self.vars_to_write:
                        ...
                        if varout not in [
                            "trajectory"
                        ]:  # because 'trajectory' is written as coordinate
                            if self.write_once(var):
                                data = np.full(
                                    (arrsize[0],), np.nan, dtype=self.vars_to_write[var]
                                )
                                data[ids_once] = pset.collection.getvardata(
                                    var, indices_to_write_once
                                )
                                dims = ["trajectory"]
                            else:
                                data = np.full(
                                    arrsize, np.nan, dtype=self.vars_to_write[var]
                                )
                                data[ids, 0] = pset.collection.getvardata(
                                    var, indices_to_write
                                )
                                dims = ["trajectory", "obs"]
                            ds[varout] = xr.DataArray(
                                data=data, dims=dims, attrs=attrs[varout]
                            )
                            ds[varout].encoding["chunks"] = (
                                self.chunks[0] if self.write_once(var) else self.chunks
                            )
                    ...
                    for var in self.vars_to_write:
                        ...
                        if self.maxids > Z[varout].shape[0]:
                            self._extend_zarr_dims(
                                Z[varout], store, dtype=self.vars_to_write[var], axis=0
                            )
                        if self.write_once(var):
                            if len(ids_once) > 0:
                                Z[varout].vindex[ids_once] = pset.collection.getvardata(
                                    var, indices_to_write_once
                                )
                        else:
                            ...
                            Z[varout].vindex[ids, obs] = pset.collection.getvardata(
                                var, indices_to_write
                            )
