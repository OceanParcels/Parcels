...
__all__ = ['ParticleSetNodes', 'BenchmarkParticleSetNodes']
...
class ParticleSetNodes(BaseBenchmarkParticleSet):
    ...
    def _get_time_bounds_(self, endtime, runtime, dt):
        ...
        starttime = min([n.data.time for n in self._collection.data if not np.isnan(n.data.time)]) if dt >= 0 else max([n.data.time for n in self._collection.data if not np.isnan(n.data.time)])
        _fieldstarttime = mintime if dt >= 0 else maxtime
        starttime = _fieldstarttime if starttime is None or np.isnan(starttime) else starttime
        ...
        _starttime = starttime[0] if isinstance(starttime, np.ndarray) or type(starttime) in [list, tuple] else starttime
        ndata = self._collection.begin()
        while ndata is not None:
            # ==== we need to skip here deleted nodes that have been queued for deletion, but are still bound in memory ==== #
            if not ndata.isvalid():
    ...
    def _write_field_data_(self, output_file):
        ...
        for fld in self._fieldset.get_fields():
            if hasattr(fld, 'to_write') and fld.to_write:
                ...
                fldfilename = str(output_file.name).replace('.nc',
                                                            '_%.4d' % fld.to_write)  # what does this do ? the variable is boolean, then it's increased - what-the-frog ...
...
class BenchmarkParticleSetNodes(ParticleSetNodes):
