...
def _create_convert_time_(time=None, fieldset=None, arrsize=None):
    ...
    assert arrsize == len(time), (
        "time [{}] and positions (lon, lat, depth) [{}] don''t have the same lengths.".format(len(time), arrsize))
    return time
...
class ParticleSetSOA(BaseBenchmarkParticleSet):
    ...
    def __init__(self, fieldset=None, pclass=JITParticle, lon=None, lat=None,
        ...
        time = _convert_to_array(time)
        time = np.repeat(time, lon.size) if len(time) == 1 else time
